    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            ...
            if result_line_number:
                found_results = True
                print(f"{Colors.red}File : {file_name}")
                print(f"{Colors.red}Result : {result_line}")
                print()
...
def main():
    ...
    search_term = input(f"{Colors.red}Enter search term : ").strip()
    ...
    try:
        ...
        if not found_results:
            print(f"{Colors.red}No matches found for '{search_term}'.")
        print(f"{Colors.red}Total files checked: {total_files}")
    except Exception as e:
